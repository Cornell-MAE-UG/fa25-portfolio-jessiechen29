...
In this design problem, we have one system which can generate three equations, but we have bar length, lifting height, mass, and reactions at the two pins which are more variables that we can solve with three equations, so for the sake of solving this problem, I set lifting height to the maximum allowable heigth which is the hight of the design space (50cm) and solve the rest of the problem with height as a given quantity.
